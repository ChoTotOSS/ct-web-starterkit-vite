...
2. **Include Pattern**: Must cover both app code and @chotot/clad-ui node_modules
3. **JSX Loader**: @chotot/clad-ui's `.js` files contain JSX, so we need `loader: { '.js': 'jsx' }`
4. **Theme Aliasing**: `@@chotot/clad-ui/theme` → `./src/theme/index.ts`
5. **Dependencies**: Add `lodash` as @chotot/clad-ui uses it
...
3. **Missing dependencies**: Added `lodash` which @chotot/clad-ui depends on
4. **Theme resolution**: Properly aliased `@@chotot/clad-ui/theme` path
